...
/** `/app/pay/return?order=<id>&status=ok|failed` — where the payment callback sends the browser back. */
export function PayReturnScreen() {
  ...
  return (
    <Screen
      title={o && !order.isPlaceholderData ? (ok ? 'پرداخت موفق' : 'پرداخت ناموفق') : 'نتیجه پرداخت'}
      subtitle={o ? `سفارش ${fa(o.code)}` : undefined}
      icon={CreditCard}
      tone={!o || order.isPlaceholderData ? 'blue' : ok ? 'green' : 'pink'}
      back="/app/orders"
    >
      {!orderId ? (
        <EmptyState
